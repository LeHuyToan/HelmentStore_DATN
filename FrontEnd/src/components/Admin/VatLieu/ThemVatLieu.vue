...
<template>
    ...
    <Dialog v-model:visible="productDialog" :style="{ width: '600px' }" header="Thêm vật liệu" :modal="true" class="p-fluid">
        ...
        <Dialog v-model:visible="addProductDialog" :style="{ width: '450px' }" header="Confirm" :modal="true">
            ...
            <template #footer>
                <Button label="HỦy" icon="pi pi-times" class="p-button-text" @click="addProductDialog = false" />
                <Button label="Đồng ý" icon="pi pi-check" class="p-button-text" @click="addProduct" />
            </template>
        </Dialog>
        <template #footer>
